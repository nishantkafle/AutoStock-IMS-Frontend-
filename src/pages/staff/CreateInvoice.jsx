...
export default function CreateInvoice() {
  ...
  const handleCheckout = async (shouldSendEmail = false) => {
    if (!customer.phone || !customer.name) {
      setMessage({ text: "Customer selection is required.", type: "error" });
      return;
    }
        ...
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "6px" }}>
          ...
          <div style={{ padding: "20px" }}>
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", marginBottom: "8px", fontSize: "13px", fontWeight: "600", color: "var(--text-muted)" }}>Select Registered Customer</label>
              <select
                value={customer.id}
            ...
            {customer.name && (
              <div style={{ padding: "12px", background: "var(--bg)", borderRadius: "8px", border: "1px solid var(--card-border)" }}>
                <div style={{ fontSize: "14px", fontWeight: "600" }}>{customer.name}</div>
                <div style={{ fontSize: "13px", color: "var(--text-muted)" }}>{customer.phone} | {customer.email}</div>
              </div>
            )}
          </div>
        </div>
